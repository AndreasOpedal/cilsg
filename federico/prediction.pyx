...
class SGDbox(AlgoBase):
    ...
    def __init__(self, n_factors=20, n_epochs=20, lr_all=0.005, reg_all=0.02, lr_pu=None, lr_qi=None, reg_pu=None, reg_qi=None, low=1, high=5, conf=None, verbose=False):
        '''
        Initializes the class with the given parameters.
        ...
        n_factors (int): the number of latent features. By default 20
        n_epochs (int): the number of iterations. By default 20
        lr_all (float): the learning rate for all parameters. By default 0.005
        reg_all (float): the regularization term for all parameters. By default 0.02
        lr_pu (float): the learning rate for P[u], takes precedence over lr_all. By default None
        lr_qi (float): the learning rate for Q[i], takes precedence over lr_all. By default None
        reg_pu (float): the regularization term for P[u], takes precedence over reg_all. By default None
        reg_qi (float): the regularization term for Q[i], takes precedence over reg_all. By default None
    ...
    def sgd(self):
        ...
        cdef double min_entry, max_entry
        cdef int u, i, f
        ...
        min_entry = np.sqrt(self.low/self.n_factors)
        max_entry = np.sqrt(self.high/self.n_factors)

        # Initialize P, Q (while keeping entries in range)
        P = np.random.uniform(min_entry, max_entry, (self.trainset.n_users,self.n_factors))
        Q = np.random.uniform(min_entry, max_entry, (self.trainset.n_items,self.n_factors))
        ...
        for current_epoch in range(self.n_epochs):
            ...
            for u, i, r in self.trainset.all_ratings():
                ...
                # Clip the vectors
                for f in range(self.n_factors):
                    np.clip(P[u,f], min_entry, max_entry)
                    np.clip(Q[i,f], min_entry, max_entry)
